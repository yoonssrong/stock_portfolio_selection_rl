...
class CpStockChart:
    ...
    def Request(self, code, objSeries):
        ...
        self.objStockChart.SetInputValue(1, ord('2'))  # 갯수로 조회
        self.objStockChart.SetInputValue(4, 100)  # 최근 100일치
        ...
        output = pd.DataFrame(columns=('date', 'open', 'high', 'low', 'close', 'vol'))
        ...
        for i in range(len):
            ...
            output = output.append(
                pd.DataFrame([[day, open, high, low, close, vol]],
                             columns=['date', 'open', 'high', 'low', 'close', 'vol']),
                ignore_index=True)
        output.to_csv('./data/raw/{}.csv'.format(code), index=False)
        print(output)
...
class CpIndexTest:
    def __init__(self):
        ...
        self.objChart.Request('A000660', self.objSeries)
...
class MyWindow(QMainWindow):
    ...
    def OnComboChanged(self):
        ...
        for key, values in chartData.items():
            print(key)
            print(values)
            df = pd.DataFrame({"{}".format(key): values})
            df.to_csv("./data/raw/{}.csv".format(key), index=False)
